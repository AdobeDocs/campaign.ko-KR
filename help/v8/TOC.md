...
+ 릴리스 정보 {#releases}
   ...
   + {hide-from-toc}[알려진 문제](start/known-issues.md)
   + [호환성 매트릭스](start/compatibility-matrix.md)
   + {hide-from-toc}[설명서 업데이트](start/documentation-updates.md)
+ 시작 {#new}
...
+ 메시지 보내기 {#send}
   ...
   + SMS {#sms}
      ...
      + [SMS 커넥터 유형 기본 정보](send/sms/sms-connectors.md)
      ...
      + SMS 채널 구성 {#config-sms}
         ...
         + {hide-from-toc}[독립형 인스턴스](send/sms/sms-standalone-instance.md)
...
+ 프로필 및 대상자 {#audience}
   ...
   + 대상자 만들기 {#create-audiences}
      ...
      + {hide-from-toc}[Adobe 솔루션과 대상 공유](start/shared-audiences.md)
   + {hide-from-toc}[모범 사례](audiences/audiences-best-practices.md)
...
+ 오퍼 관리 - 상호 작용 {#offers}
   ...
   + [사용 사례](interaction/interaction-use-cases.md)
+ 추적 및 분석 {#analytics}
   + 메시지 추적 {#tracking}
...
+ 구성 {#config}
   ...
   + 설정 {#settings}
      ...
      + {hide-from-toc}[Campaign SDK를 앱과 통합 - 더 이상 사용되지 않는 페이지](config/push-config.md)
...
+ 개발자 리소스 {#developer}
   ...
   + {hide-from-toc}[queryDef를 사용하여 데이터베이스 쿼리](dev/query-api.md)
   + REST API {#apis}

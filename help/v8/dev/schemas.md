...
* 응용 프로그램 내의 데이터 객체가 기본 데이터베이스 테이블에 연결되어 있는 방식을 정의합니다.
...
>[!CAUTION]
>
>일부 내장 캠페인 스키마에는 클라우드 데이터베이스에 연관된 스키마가 있습니다. 이러한 스키마는 **Xxl** 네임스페이스로 식별되므로 수정할 수 없습니다.
...
특정 네임스페이스는 Adobe Campaign 응용 프로그램 작업에 필요한 시스템 엔터티에 대한 설명을 위해 예약되어 있습니다. 다음 네임스페이스 **은(는) 대문자/소문자 조합에서 새 스키마를 식별하기 위해 사용할 수 없습니다.**

* **xxl**:클라우드 데이터베이스 스키마에 예약됨,
* **xtk**:플랫폼 시스템 데이터에서 예약됨
* **nl**:응용 프로그램의 전체 사용에 대해 예약됨,
* **nms**:배달로 예약됨(수신자, 배달, 추적 등),
* **ncm**:컨텐츠 관리에 예약됨
* **임시**:임시 스키마로 예약됨.

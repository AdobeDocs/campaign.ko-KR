...
Campaign 기본 제공 표와 표 간의 관계를 더 잘 이해하려면 [이 섹션](datamodel.md)을 참조하세요.

Campaign 스키마를 시작하려면 [이 섹션](schemas.md)을(를) 읽으십시오.

[이 페이지](extend-schema.md)에서 Adobe Campaign 데이터베이스의 개념 데이터 모델을 확장하기 위해 확장 스키마를 구성하는 방법을 알아봅니다.
...
>[!NOTE]
>
>Adobe Campaign에서 [사용자 지정 받는 사람 테이블](custom-recipient.md)을 만들 수 있습니다. 그러나 대부분의 경우 이미 사전 빌드된 추가 테이블 및 기능이 있는 기본 제공 [받는 사람 테이블](datamodel.md#ootb-profiles)을 활용하는 것이 좋습니다.
...
* **문자열** 필드의 길이는 항상 열을 사용하여 정의해야 합니다. 기본적으로 Adobe CampaignAdobe 의 최대 길이는 16K이지만, 크기가 더 짧은 길이를 초과하지 않도록 이미 알고 있는 경우에는 필드를 더 짧게 유지하는 것이 좋습니다.
...
대부분의 테이블에서 기본적으로 정의된 **autuuid** 및 **autopk** 외에 일부 논리 또는 비즈니스 키(계정 번호, 클라이언트 번호 등)를 추가하는 것이 좋습니다. 나중에 가져오기/조정 또는 데이터 패키지에 사용할 수 있습니다. 자세한 내용은 [식별자](#identifiers)를 참조하세요.

효율적인 키는 성능을 위해 필수적입니다. Snowflake을 사용하면 숫자 또는 문자열 기반 데이터 유형을 표의 키로 삽입할 수 있습니다.

>[!NOTE]
>
>**autuuid** 특성은 [엔터프라이즈(FFDA) 배포](../architecture/enterprise-deployment.md)에만 적용됩니다.
...
[엔터프라이즈(FFDA) 배포](../architecture/enterprise-deployment.md)의 컨텍스트에서 Adobe Campaign 기본 키는 모든 기본 제공 테이블에 대해 자동 생성된 UUID입니다. UUID는 사용자 지정 테이블에도 사용할 수 있습니다. [자세히 알아보기](../architecture/keys.md)
...
* 기본 키를 외부 시스템 기본 키로 사용. 이 솔루션은 서로 다른 시스템 간에 일관된 키를 사용하여 데이터를 가져오고 내보내는 접근 방식을 단순화하기 때문에 일반적으로 선호됩니다. 키 이름이 &quot;id&quot;이고 외부 값으로 채워야 하는 경우 **Autuuid**&#x200B;을(를) 비활성화해야 합니다(자동 생성되지 않음).
...
링크를 외부 조인으로 선언하는 것은 성능에 좋지 않습니다. 제로 ID 레코드는 외부 조인 기능을 에뮬레이션합니다. [엔터프라이즈(FFDA) 배포](../architecture/enterprise-deployment.md)의 컨텍스트에서는 링크가 **autuuid**&#x200B;을(를) 사용하는 경우 외부 조인을 선언할 필요가 없습니다.
...
역방향 링크가 사용자에게 표시되지 않아야 하는 경우 링크 정의 revLink=&#39;_없음_&#39;을 사용하여 숨길 수 있습니다. 이에 대한 유용한 사용 사례는 수신자로부터 예를 들어 완료된 마지막 거래로의 링크를 정의하는 것입니다. 수신자에서 마지막 트랜잭션으로의 링크만 표시하면 되며, 트랜잭션 테이블에서 역방향 링크를 볼 필요는 없습니다.
...
보존과 관련하여 Campaign의 기본 제공 로그인 로그 테이블에는 사전 설정된 보존 기간이 있으며 일반적으로 데이터 저장소를 6개월 이내로 제한합니다.
...
Managed Cloud Services 사용자는 Adobe 컨설턴트 또는 기술 관리자에게 문의하여 보존에 대한 자세한 내용을 확인하거나 사용자 지정 테이블에 대한 보존을 설정해야 합니다.

...
1. 웹 페이지가 표시되는 동안 스크립트 평가를 통해 수신자 ID를 오퍼 엔진에 전달할 수 있습니다. ID가 합성인 경우, 키는 고급 설정에 사용된 것과 동일한 순서로 표시되며 |로 구분됩니다.
...
* **상태**: 문자열 형식이며 오퍼의 새 상태를 지정합니다. 가능한 값은 **nms:common** 스키마의 **propositionStatus** [enumeration](../config/enumerations.md)에 나열됩니다. 예를 들어, 기본적으로 숫자 3은 **수락됨** 상태에 해당합니다.

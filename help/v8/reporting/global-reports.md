...
* **[!UICONTROL URLs and click streams]** : 참조 [URL 및 클릭 스트림](delivery-reports.md#urls-and-click-streams).
* **[!UICONTROL Tracking indicators]** : 참조 [지표 추적](delivery-reports.md#tracking-indicators).
...
* **[!UICONTROL Delivery summary]** : 참조 [게재 요약](delivery-reports.md#delivery-summary).
...
>[!NOTE]
>
>이 보고서에 표시된 오류는 격리 프로세스를 트리거합니다. 격리 관리에 대한 자세한 내용은 [격리 관리](../send/quarantines.md).
...
* **[!UICONTROL Not connected]** : 전송 시 수신자의 휴대 전화가 꺼져 있거나 네트워크 연결이 끊겼음을 나타내는 오류 유형입니다.
   ...
   >[!NOTE]
   >
   >이 지표는 게재 시 해당됩니다 [모바일 채널](../send/send.md) 전용.
...
>[!NOTE]
>
>이 보고서에 표시되는 도메인 이름은 큐브 수준에서 정의됩니다. 이러한 값을 변경하려면 **[!UICONTROL Delivery logs (broadlogrcp)]** 큐브입니다. 이 작업에 대한 자세한 정보는 [이 섹션](gs-cubes.md)을 참조하십시오. 다음 **[!UICONTROL Others]** 카테고리에는 특정 클래스에 속하지 않는 도메인 이름이 포함되어 있습니다.
...
자세한 내용은 [이 페이지](../start/subscriptions.md).
...
열기 수는 열린 총 메시지 수에 해당합니다. 텍스트 형식 이메일은 카운트되지 않습니다. 열기 추적에 대한 자세한 내용은 [이 섹션](metrics-calculation.md#tracking-opens-).

...
트랜잭션 메시지 설정에 대한 자세한 내용은 [이 섹션](../config/transactional-msg-settings.md).
...
날짜 [메시지 템플릿 만들기](#create-message-template)를 클릭하고, 보낼 메시지와 일치하는 이벤트 유형을 선택합니다.
...
1. 다음으로 이동 **[!UICONTROL Administration > Platform > Enumerations]** Campaign 탐색기 폴더.
1. 다음 항목 선택 **[!UICONTROL Event type]** 목록에서 열거합니다.
...
1. 특성 및 설명을 입력한 다음 **[!UICONTROL Continue]** 메시지 본문을 작성합니다.
...
1. 클릭 **[!UICONTROL Ok]** 시드 주소 만들기를 확인합니다.
...
1. 메시지 템플릿에서 **[!UICONTROL Preview]** 탭을 선택한 다음 를 선택합니다 **[!UICONTROL A seed address]** 을 클릭합니다.
...
증명 보내기에는 게재의 프로세스와 동일한 프로세스가 포함됩니다. 에서 증명에 대해 자세히 알아보기 [이 섹션](../send/preview-and-proof.md).
...
메시지 템플릿을 만들 때<!-- on the control instance--> 이(가) 완료되면 게시할 수 있으며, 이렇게 하면 실시간 및 배치 이벤트에 연결된 메시지를 보낼 수 있습니다.
...
1. 로 이동 **[!UICONTROL Message Center > Transactional message templates]** 트리의 폴더입니다.
1. 게시할 템플릿 선택<!--on your execution instances-->.
1. **[!UICONTROL Publish]**&#x200B;를 클릭합니다.

   ![](assets/messagecenter_publish_template.png)

게시가 완료되면 일괄 처리에 적용할 메시지 템플릿과 실시간 유형 이벤트가에 모두 만들어집니다. **[!UICONTROL Administration > Production > Message Center Execution> Default > Transactional message templates]** 폴더를 삭제합니다.

![](assets/messagecenter_deployed_model.png)

템플릿이 게시되면 해당 이벤트가 트리거되면 Adobe Campaign<!--execution instance--> 은 이벤트를 수신하고 트랜잭션 템플릿에 연결하며 각 수신자에게 해당 트랜잭션 메시지를 보냅니다.
...
메시지 템플릿이 게시되면 <!--on the execution instances-->, 게시를 취소할 수 있습니다.
...
1. 다음으로 이동 **[!UICONTROL Message Center > Transactional message templates]** 폴더를 삭제합니다.

...
1. 다음에서 **[!UICONTROL Remote object]** 드롭다운 목록에서 가져올 객체를 선택합니다. 이 개체는 커넥터 구성 중에 Adobe Campaign에서 만든 표 중 하나와 일치합니다.
1. 다음에서 **[!UICONTROL Remote fields]** 섹션에서 가져올 필드를 입력합니다.

   필드를 추가하려면 **[!UICONTROL Add]** 도구 모음에서 단추를 클릭한 다음 **[!UICONTROL Edit expression]** 아이콘.

   필요한 경우 의 드롭다운 목록을 사용하여 데이터 형식을 변경합니다. **[!UICONTROL Conversion]** 열. 가능한 전환 유형은에 자세히 설명되어 있습니다. [이 섹션](#data-format).
...
1. 필요에 따라 가져올 데이터를 필터링할 수 있습니다. 이렇게 하려면 **[!UICONTROL Edit the filter...]** 링크를 클릭합니다.
   ...
   >[!CAUTION]
   >
   >데이터 필터링 모드와 관련된 제한 사항은에 자세히 설명되어 있습니다. [이 섹션](#filtering-data).

1. 다음 항목 선택 **[!UICONTROL Use automatic index...]** 날짜와 마지막 수정 사항에 따라 CRM과 Adobe Campaign 간의 증분 개체 동기화를 자동으로 관리하는 옵션입니다.
...
1. 로 이동 **[!UICONTROL Remote object]** 드롭다운 목록을 표시하고 내보낼 객체를 선택합니다. 이 개체는 커넥터 구성 중에 Adobe Campaign에서 만든 표 중 하나와 일치합니다.

   >[!CAUTION]
   >
   >의 내보내기 기능 **[!UICONTROL CRM Connector]** 활동은 CRM의 필드를 삽입하거나 업데이트할 수 있습니다. CRM에서 필드 업데이트를 활성화하려면 원격 테이블의 기본 키를 지정하십시오. 키가 없으면 데이터가 업데이트되지 않고 삽입됩니다.

1. 더 빠른 내보내기를 수행해야 하는 경우  **[!UICONTROL Export in Batches]** 옵션을 선택합니다.

   ![](assets/crm-export-batch.png)

1. 다음에서 **[!UICONTROL Mapping]** 섹션, 클릭 **[!UICONTROL New]** CRM에서 내보낼 필드 및 해당 매핑을 지정합니다.
...
* **[!UICONTROL Without time offset]**: 기본 모드에 적용된 시간대 관리를 취소합니다.
* **[!UICONTROL Copy/Paste]**: 문자열과 같은 원시 데이터를 사용합니다(전환 없음).

### 오류 처리 중 {#error-processing}

데이터 가져오기 또는 내보내기 프레임워크 내에서 특정 프로세스를 오류와 거부에 적용할 수 있습니다. 이렇게 하려면 **[!UICONTROL Keep the rejections in a file]** 및 **[!UICONTROL Process errors]** 의 옵션 **[!UICONTROL Behavior]** 탭.
...
그런 다음 데이터를 처리할 관련 활동을 삽입합니다. 예를 들어 **대기** 오류에 대한 활동 및 다시 시도 예약.
...
1. 에서 고려할 삭제 기간을 지정합니다. **[!UICONTROL Start date]** 및 **[!UICONTROL End date]** 필드(날짜 포함).

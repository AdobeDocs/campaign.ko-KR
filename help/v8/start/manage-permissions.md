...
1. 다음에서 [Admin Console](https://adminconsole.adobe.com/enterprise){target="_blank"} 홈 페이지, 선택 **사용자 추가**.
...
1. &#39;+&#39; 기호를 사용하여 사용자에게 할당할 제품 프로필 또는 사용자 그룹을 선택합니다.
   ...
   캠페인 기본 제공 제품 프로필은에 나열되어 있습니다. [이 섹션](#ootb-productprofiles).
...
새 사용자 [campaign에 로그온](connect.md) Adobe ID을 사용하면 클라이언트 콘솔의 Campaign 연산자 목록에 추가됩니다. 캠페인 연산자는 **[!UICONTROL Administration > Access management > Operators]** Campaign 탐색기 폴더.
...
이러한 제품 프로필은에 저장된 연산자 그룹과 일치합니다. **[!UICONTROL Administration > Access management > Operator groups]** Campaign 탐색기 폴더.

Admin Console에서 제품 프로필은 다음 구문을 사용합니다.

캠페인 - `<your instance>` - 연산자 그룹의 내부 이름

예: **게재 연산자** Admin Console의 제품 프로필은 &quot;test&quot; 인스턴스의 그룹입니다.
...
1. Campaign에 연결하여 탐색기를 열고 다음 위치로 이동합니다. **[!UICONTROL Administration > Access management > Operator groups]**.
1. 클릭 **[!UICONTROL New]**연산자 그룹의 이름을 정의하고 내부 이름(&#39;검토자&#39;)을 설정합니다.
   ![](assets/new-op-group.png)
1. 명명된 권한을 선택하여 관련 권한을 정의합니다. 명명된 권한에 대해서는 다음에서 자세히 설명합니다 [이 섹션](#use-named-rights)
1. 새 연산자 그룹을 저장합니다.
...
1. 다음에서 **제품 및 서비스** 홈 페이지의 섹션에서 Campaign 제품을 엽니다.
1. 클릭 **새 프로필** 설명된 대로 정확한 구문을 사용하여 생성할 제품 프로필의 이름을 입력합니다 [여기](#product-profiles). 이 예제에서는 campaign - `<your-instance-name>` - 검토자

   ![](assets/new-product-profile-ui.png)

1. 변경 내용을 저장합니다.

에 설명된 대로 이제 사용자를 이 새 제품 프로필에 추가할 수 있습니다. [이 섹션](#add-users).
...
Adobe Campaign에는 내장 기능이 포함되어 있습니다 **제품 프로필** Adobe이 환경을 활성화하면 정의됩니다.

![](assets/ootb-product-profiles.png)

이 제품 프로필은 Campaign과 일치합니다. **연산자 그룹**. 기본 연산자 그룹 및 [명명된 권한](#use-named-rights) 아래에 나열되어 있습니다.
...
1. **[!UICONTROL Content contributors]** (콘텐츠)
   ...
   이 그룹의 사용자는 의 컨텍스트에서 콘텐츠 폴더에 액세스할 수 있습니다. **[!UICONTROL Content management]** 추가 기능. 이 그룹은 추가 권한을 부여하지 않습니다.

1. **[!UICONTROL Access to reports]** (보고서)

   이 그룹은 외부 운영자가 을(를) 통해 게재 보고서에 액세스하도록 예약되어 있습니다. [웹 액세스](../start/campaign-ui.md#web-browser).
...
1. **[!UICONTROL Offer managers]** (오퍼)
   ...
   * 오퍼를 승인합니다.
   ...
   환경당 오퍼 관리자 권한 매트릭스는에서 사용할 수 있습니다. [이 페이지](../interaction/interaction-operators.md#recap-of-rights-according-to-operator).

## 사용자 그룹 작업{#user-groups}

Admin Console을 사용하여 사용자 그룹을 만들고 사용자를 할당할 수 있습니다.

사용자 그룹은 공유 권한 집합을 지정해야 하는 여러 사용자의 컬렉션입니다. 에서 사용자 그룹을 만드는 방법 알아보기 [이 섹션](https://helpx.adobe.com/ie/enterprise/using/user-groups.html){target="_blank"}.

사용자 그룹에 제품 프로필을 할당할 수 있습니다. 따라서 해당 그룹의 모든 사용자는 동일한 제품 권한 집합을 받게 됩니다.

## 명명된 권한{#use-named-rights}

Adobe Campaign에는 사용자 및 사용자 그룹에 할당된 권한을 정의할 수 있는 명명된 권한 집합이 포함되어 있습니다. 이러한 권한은 다음에서 편집할 수 있습니다 **[!UICONTROL Administration > Access management > Named rights]** Campaign 탐색기 폴더.

명명된 권한 부여 대상:

* 작업 수행 예: **분석** 게재 편집기의 버튼이 구성원에 대해 활성화됩니다. **게재 연산자** 이(가) 있는 그룹 **게재 준비** 명명된 권한

* 폴더 액세스 운영자 그룹의 멤버쉽은 폴더에 대한 보안 설정을 변경하여 폴더에 대한 액세스 권한을 부여하거나 제한할 수 있습니다. [자세히 알아보기](folder-permissions.md#restrict-access-to-a-folder)

  예를 들어 다음 항목에 영향을 줄 수 있습니다. **쓰기 액세스 권한** 게재, 프로필 등과 같은 새 엔티티를 만들려면 **읽기 액세스** 엔티티를 사용하려면 **액세스 삭제** 엔티티를 삭제합니다.
...
* **[!UICONTROL USE SQL DATA MANAGEMENT ACTIVITY]**: 작업 테이블을 만들고 채우기 위해 SQL 데이터 관리 활동을 사용하여 고유한 SQL 스크립트를 작성할 권한. [자세히 알아보기](../../automation/workflow/sql-data-management.md)

* **[!UICONTROL WORKFLOW]**: 이 명명된 권한은 워크플로우에 따라 다르며, 워크플로우를 만들고 시작하고 중지할 수 있습니다. 지정된 권한을 적용하려면 워크플로우 파일에 대한 읽기 권한이 필요합니다. 타겟팅 워크플로우의 경우 **[!UICONTROL Profiles and Targets]** 폴더가 필요합니다.

...
* 모니터링 및 테스트 목적으로, 이제 **[!UICONTROL Replicate Staging data]** 워크플로의 감사 로그에 FFDA(Full Federated Data Access) 데이터베이스로 보낸 레코드의 수가 포함됩니다.
...
* 워크플로에서 강화 활동으로 관리하는 오퍼가 게재에 삽입되지 않는 문제를 해결했습니다.
...
<thead>
<tr>
<th><strong>새로운 워크플로 활동: 데이터 소스 변경</strong><br/></th>
</tr>
</thead>
<tbody>
<tr>
<td>
<p>새로 추가된 <b>데이터 소스 변경</b> 워크플로 활동을 사용하면 워크플로의 작업 테이블의 데이터 소스를 변경할 수 있습니다. 이를 통해 다양한 데이터 소스(FDA, FFDA 및 로컬 데이터베이스)에서 데이터를 유연하게 관리할 수 있습니다.</p>
<p>Adobe Campaign 워크플로에서 데이터는 작업(또는 임시) 테이블을 사용하여 관리됩니다. 워크플로를 실행하면 작업 테이블이 여러 워크플로 활동 간에 데이터를 공유합니다. 기본 설정에서 작업 테이블은 쿼리하는 데이터의 소스와 동일한 데이터베이스에 생성됩니다.</p>

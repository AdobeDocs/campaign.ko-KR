...
* Campaign v8.4 - [릴리스 8.4.1](#release-8-4-1) | [릴리스 8.4.2](#release-8-4-2)
* Campaign v8.3 - [릴리스 8.3.8](#release-8-3-8) | [릴리스 8.3.9](#release-8-3-9)
* Campaign v8.2 - [릴리스 8.2.10](#release-8-2-10)
...
<p>자세한 내용은 <a href="../connect/ac-aep.md">세부 설명서</a>를 참조하십시오.</p>
</td> 
</tr> 
</tbody> 
</table>
...
* 워크플로 고가용성을 사용하여 워크플로 실행을 개선하여 여러 컨테이너에서 동시에 워크플로를 실행하여 워크플로 서비스 손실과 관련 실행 오류를 방지할 수 있습니다. **참고**: 이 새로운 기능은 일부 고객에게만 제한된 가용성으로 제공됩니다.
...
* **데이터 로드(파일)** 활동을 사용할 때 서버에서 파일이 업데이트되지 않는 워크플로 문제를 해결했습니다. 프로세스가 100%에서 중단되었지만 종료되지 않았습니다. (NEO-47269)
...
* 실행 중 오류가 발생한 경우에도 **[!UICONTROL Replicate Staging data]**(ffdaReplicateStagingData) 기술 워크플로가 중지되지 않는 문제를 해결했습니다. (NEO-46280)
* 시드 주소로 교정쇄를 보낼 때 속도가 느려지지 않도록 시드 멤버의 모든 연속 복제가 이제 하나의 복제 요청으로 그룹화됩니다. (NEO-44844)
...
* 이제 다음 **데이터베이스 정리** 기술 워크플로가 사용자 지정 스테이징 스키마도 처리합니다. (NEO-48974)
...
* 스테이징 워크플로가 오류이고 보존 기간이 완전히 지난 경우 데이터 손실이 발생할 수 있는 문제를 해결했습니다. (NEO-48975)
...
* FeatureFlag_GZIP_Compression 옵션이 활성화된 경우 MID 인스턴스에서 게재 로그 상태 업데이트에 영향을 주는 문제를 수정했습니다. (NEO-49183)
* 이제 **데이터베이스 정리** 기술 워크플로가 사용자 지정 스테이징 스키마도 처리합니다. (NEO-48974)
...
<tbody> 
<tr> 
<td> <p>이제 Campaign v8이 Adobe 개인 정보 보호 핵심 서비스와 통합됩니다. 개인 정보 보호 핵심 서비스에서 모든 Experience Cloud 솔루션으로 푸시된 개인 정보 보호 요청은 전용 워크플로를 통해 Campaign에서 자동으로 처리됩니다.</p>
...
<tbody> 
<tr> 
<td> <p>Campaign Distributed Marketing을 사용하면 중앙 엔터티(본사, 마케팅 부서 등)와 로컬 엔터티(영업 지점, 지역 에이전시 등) 간의 공동 작업 캠페인을 구현할 수 있습니다. 공유 작업 공간(캠페인 패키지)을 통해 캠페인 템플릿을 만들고 로컬 엔티티에 제안할 수 있습니다.</p>
...
* 사용자 지정 스키마를 대상 매핑으로 사용할 때 추적 워크플로가 실패하는 문제를 해결했습니다. 이제 대상 매핑 마법사를 통해 broadLog 스키마를 생성할 때 사용자 지정 타깃팅 스키마에 대한 외부 링크 유형이 올바른지 확인합니다. (NEO-43506)
* 영어 이외 언어에 대해 FFDA 배포 워크플로가 실패할 수 있는 문제를 해결했습니다. (NEO-44561)
...
* 시드 또는 대체 주소로 교정쇄를 보낼 때 발생하는 ID 생성 문제를 해결했습니다. (NEO-42637)
* 주소 대체 방법을 사용하여 교정쇄를 보낼 수 없는 문제를 해결했습니다. (NEO-40417)

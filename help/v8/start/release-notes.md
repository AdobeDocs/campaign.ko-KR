...
* 유형화 규칙에 정의된 최대 메시지 수에 도달하면 게재 준비가 실패하는 문제를 해결했습니다.
* 이메일 주소에 &quot;s&quot; 문자가 포함되어 있을 때 Adobe Analytics 커넥터를 구성하는 동안 발생하는 문제를 해결했습니다.
* 업그레이드 후 전달 매핑 테이블에서 사용자 지정 게재 매핑의 데이터를 잃게 하는 문제를 해결했습니다.
* 이메일 주소에 따옴표 문자(&#39;)가 포함된 경우 수신자가 동일한 게재에 대해 동일한 메시지를 여러 번 받을 수 있는 문제를 해결했습니다. 이제 이 문자를 이스케이프 처리합니다. (NEO-41198)
* 시드 또는 대체 주소로 증명을 전송할 때 발생하는 ID 생성 문제를 해결했습니다. (NEO-42637)

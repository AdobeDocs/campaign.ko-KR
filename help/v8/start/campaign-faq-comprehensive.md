...
Campaign v8.6 릴리스부터 중앙 Adobe Experience Cloud 환경을 통해 사용할 수 있는 **Campaign 웹 사용자 인터페이스**&#x200B;에 액세스할 수 있습니다. Experience Cloud는 Adobe의 디지털 마케팅 애플리케이션, 제품 및 서비스 통합 제품군입니다.

[이 페이지에서](campaign-ui.md#ac-web-ui) Adobe Experience Cloud에 연결하여 Adobe Campaign Web 인터페이스에 액세스하는 방법을 알아보십시오. [Adobe Campaign Web 사용자 인터페이스 설명서](https://experienceleague.adobe.com/ko/docs/campaign-web/v8/campaign-web-home){target="_blank"}에서 자세히 알아보십시오.
...
[Campaign 버전 및 업그레이드](upgrades.md) | [클라이언트 콘솔 업그레이드](connect.md#upgrade-ac-console) | [릴리스 노트](release-notes.md)
...
[Campaign v8 주요 기능](whats-new.md) | [Campaign v8 아키텍처](../architecture/architecture.md) | [기능 매트릭스](https://experienceleague.adobe.com/ko/docs/campaign-web/v8/start/capability-matrix){target="_blank"} | [보호 및 제한 사항](ac-guardrails.md)
...
[Campaign Classic v7에서 v8로](v7-to-v8.md) | [Campaign Standard 전환 안내서](https://experienceleague.adobe.com/ko/docs/campaign-web/v8/start/acs-migration){target="_blank"} | [기능 매트릭스](https://experienceleague.adobe.com/ko/docs/campaign-web/v8/start/capability-matrix){target="_blank"}
...
* 사용자 지정 리소스 →**스키마** | 메시지 → **게재** | 제품 사용자 → **연산자**
* 보안 그룹 → **운영자 그룹** | 조직 단위 → **폴더 권한**

**Campaign 웹 UI 업데이트:**

* **→**&#x200B;을(를) 받는 사람 | 시드 주소 → **테스트 프로필** | 게재 분석 → **게재 준비**
* {→}대상&#x200B;**목록 | 전자 메일 미리 보기 →**&#x200B;콘텐츠 시뮬레이션&#x200B;**&#x200B;**
...
[기능 매트릭스](https://experienceleague.adobe.com/ko/docs/campaign-web/v8/start/capability-matrix){target="_blank"} | [v7에서 v8로의 전환 안내서](v7-to-v8.md) | [Campaign Standard에서 v8로 전환](https://experienceleague.adobe.com/ko/docs/campaign-web/v8/start/acs-migration){target="_blank"}
...
예, 하지만 **Campaign 웹 UI를 통해서만**&#x200B;됩니다. Microsoft Azure OpenAI 및 Adobe Firefly에서 제공하는 AI Assistant는 이메일, SMS 및 푸시 알림 전반에서 전문적이고 브랜드 일관적인 콘텐츠를 만드는 데 도움이 됩니다.
...
[게재 기능 시작](../send/about-deliverability.md) | [메시지 내용 제어](../send/control-message-content.md) | [게재 기능 모니터링](../send/monitoring-deliverability.md) | [SpamAssassin](../send/spamassassin.md)
...
[시드 주소 가이드](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/action-activities/delivery-control.html?lang=ko){target="_blank"}에서 자세히 알아보기
...
Adobe Campaign에는 애플리케이션 서버의 여러 모듈에 걸쳐 전체 프로세스 및 작업을 오케스트레이션하는 워크플로가 포함되어 있습니다. 이 포괄적인 그래픽 환경을 사용하면 세분화, 캠페인 실행, 파일 처리, 인원 참여 등의 프로세스를 디자인할 수 있습니다. 워크플로우 엔진은 이러한 프로세스를 실행하고 추적합니다.
...
[활동 분할](https://experienceleague.adobe.com/docs/campaign/automation/workflows/wf-activities/targeting-activities/split.html?lang=ko){target="_blank"} | [A/B 테스트 안내서](https://experienceleague.adobe.com/docs/campaign/automation/workflows/use-cases/deliveries/a-b-testing.html){target="_blank"}
...
+++ 내 운영자 및 권한이 보안 모범 사례를 따르는지 확인하려면 어떻게 해야 합니까?

Adobe Campaign v8에서는 최상위 관리 계정 및 권한 있는 계정의 현재 설정을 모두 권장 보안 기본값과 비교하는 기능을 제공합니다.

* **현재 설정(전체 가시성):** 관리 및 권한 있는 계정에 대한 모든 설정은 **[!UICONTROL Administration]** > **[!UICONTROL Access management]**(클라이언트 콘솔 또는 Campaign 웹 UI): **[!UICONTROL Operators]**, **[!UICONTROL Operator groups]**, **[!UICONTROL Named rights]**(예: 관리, 승인 관리) 및 폴더 수준 권한에서 검토할 수 있습니다. 제품 관리자는 [Campaign Campaign 컨트롤 패널](https://experienceleague.adobe.com/docs/control-panel/using/control-panel-home.html?lang=ko){target="_blank"}에서 인스턴스 수준 설정(IP 권한, URL 권한, 인스턴스 세부 정보 등)을 검토할 수도 있습니다.
* **권장 보안 기본값:** [보안 지침](../config/security.md)은(는) 충분한 보안 그룹을 만들고, 연산자별로 적절한 액세스 권한을 할당하고, 관리 연산자 및 관리 그룹의 사용을 제한하는 등 관리 및 권한 있는 계정의 권장 보안 기준을 정의합니다. 해당되는 경우 네트워크 및 암호화 강화에 대한 [향상된 보안 추가 기능](../config/enhanced-security.md)도 참조하십시오.
* **비교:** 관리자는 현재 구성을 이러한 문서화된 권장 사항과 비교하여 차이점을 식별하고 보안 기본값에 맞춥니다.

**관련 항목:**

[사용 권한 시작](gs-permissions.md) | [보안 지침](../config/security.md) | [향상된 보안 추가 기능](../config/enhanced-security.md)
...
[Campaign의 게재 기능 기본 정보](../send/about-deliverability.md) | [게재 모범 사례 안내서](https://experienceleague.adobe.com/docs/deliverability-learn/deliverability-best-practice-guide/introduction.html?lang=ko){target="_blank"}

+++

+++ 외부 데이터베이스에 저장된 데이터에 액세스하려면 어떻게 해야 합니까?

Campaign v8은 Campaign 데이터 모델을 변경하지 않고 외부 데이터베이스에 저장된 데이터를 쿼리하고 사용할 수 있도록 FDA(Federated Data Access)를 제공합니다. 외부 계정을 구성한 다음 워크플로, 쿼리 및 타겟팅에서 FDA를 사용하여 외부 데이터를 Campaign 프로필과 결합합니다.

FDA 가용성 및 연결 요구 사항은 배포 및 라이선스에 따라 다릅니다. 호환성 매트릭스에서 지원되는 데이터베이스 및 필요한 버전을 검토합니다.

**관련 항목:**

[FDA 연결 구성](../connect/fda.md) | [호환성 매트릭스](compatibility-matrix.md)
...
클라이언트 콘솔을 닫은 다음 사용자 프로필에서 로컬 캐시 파일을 수동으로 제거합니다.

**수동 캐시 정리(Windows):**

1. 클라이언트 콘솔에서 **[!UICONTROL File]** > **[!UICONTROL Clear the local cache...]**&#x200B;을(를) 선택합니다.
2. 로그아웃한 후 클라이언트 콘솔을 닫습니다.
3. Windows 버전을 기준으로 캐시 위치로 이동합니다. Windows 10의 경우: `C:\Users\<Username>\AppData\Roaming\Neolane\NL_5\`
4. `nlclient-config-<alphanumerical>.xml`개 파일 및 연결된 폴더를 삭제합니다.

>[!IMPORTANT]
>
>`nlclient_cnx.xml`을(를) 삭제하지 마십시오.

Adobe 지원에서 버전 및 OS에 대한 올바른 캐시 위치를 확인하고 안전한 수동 정리를 안내할 수 있습니다.

[클라이언트 콘솔 설치 및 구성](connect.md)에서 자세히 알아보기

+++

+++ Adobe Campaign을 LDAP와 통합할 수 있습니까?

아니요. Campaign v8은 LDAP 통합을 지원하지 않습니다. 인증은 Adobe IMS(Adobe ID 또는 Federated ID)를 사용합니다. Campaign Classic v7에서 마이그레이션하는 경우 지원되지 않는 기능 및 인증 변경 사항을 검토하십시오.

**관련 항목:**

[Campaign v8에 연결](connect.md) | [v8에서 지원되지 않는 기능](v7-to-v8.md#gs-removed)
...
* **[!UICONTROL Administration > Configuration > Data schemas]**&#x200B;을(를) 통한 **보기:** 액세스

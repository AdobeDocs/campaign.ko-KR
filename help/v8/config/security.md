...
description: Campaign에 대한 권장 보안 구성 지침
feature: Privacy, PI
role: Developer
level: Beginner
...
### 이 지침을 찾을 수 있는 위치 {#public-guidance}

Adobe Campaign은 현재 컴퓨터에서 읽을 수 있는 형식으로 권장 보안 구성 지침을 제공하지 않습니다. 다음 설명서를 사용하여 현재 설정을 권장 보안 기본값과 비교할 수 있습니다.

* **이 페이지** - [Campaign 보안 모범 사례](#ac-security)(검사 목록 및 세부 섹션)
* **[캠페인 설정(FAQ)](../start/campaign-faq-comprehensive.md#settings)** - 권장 보안 기본값과 설정 비교
* **[향상된 보안 추가 기능](enhanced-security.md)** - 보안 CMK 통합 및 보안 VPN 터널링
* **[권한 시작](../start/gs-permissions.md)** - 액세스 및 제품 프로필
* **[PII 보기 제한](../dev/restrict-pi-view.md)** - 중요한 필드에 대한 액세스 제한
* **[구현 지침](../start/implement.md)** - 시작하기 전에 보안 및 개인 정보 보호
...
개인 정보 관리는 개인 정보 보호 규정(GDPR, CPA 등)을 준수하는 데 도움이 되는 모든 프로세스 및 도구를 의미합니다.
...
Adobe Campaign을 사용하면 보낸 이메일과 게재 수신자의 동작(열기, 링크 클릭, 구독 취소 등)을 추적할 수 있습니다. 자세한 내용은 [메시지 정보](../start/gs-message.md)를 참조하세요.
...
콘텐츠에 개인화된 링크를 추가할 때 잠재적인 보안 차이를 방지하기 위해 항상 URL의 호스트 이름 부분에 개인화를 두지 마십시오. 다음 예제는 모든 URL 특성 &lt;`a href="">` 또는 `<img src="">`에서 사용해서는 안 됩니다.

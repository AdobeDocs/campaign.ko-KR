---
audience: end-user
user-guide-title: Campaign 기술 정보
type: Event
title: Adobe Campaign 테크노트
description: Campaign 기술 정보
breadcrumb-title: 캠페인 기술 정보
source-git-commit: 25ce962e7c8b6a62fc2c1edb08a78afa839d264e
workflow-type: tm+mt
source-wordcount: '88'
ht-degree: 27%

---


# Adobe Campaign 테크노트 {#technotes-ac}

+ [캠페인 기술 정보](technotes-home.md)
+ 업그레이드{#tn-new}
  + {hide-from-toc}[푸시 알림 채널 변경](upgrades/push-technote.md)
  + [사용자를 IMS로 마이그레이션](upgrades/migrate-users-to-ims.md)
  + [기술 운영자를 Developer Console로 마이그레이션](upgrades/ims-migration.md)
  + {hide-from-toc}[기술 연산자를 Developer Console으로 마이그레이션](upgrades/ims-migration-old.md)
  + [새 SMS 커넥터로 이동](upgrades/sms-migration.md)
  + {hide-from-toc}[AWS으로 Campaign 전송 인프라 마이그레이션](upgrades/migrate-to-aws.md)
  + {hide-from-toc}[Campaign 전자 메일 전송 인프라 업그레이드](upgrades/upgrade-to-aws.md)
  + {hide-from-toc}[Apache 버전 업그레이드](upgrades/apache.md)
  + {hide-from-toc}[인터넷 브라우저](upgrades/browsers.md)
  + {hide-from-toc}[2023년 시스템 업그레이드](upgrades/tech-stack-upgrade.md)
  + {hide-from-toc}[클라이언트 콘솔 32비트 사용 중단](upgrades/console.md)
  + [64비트 스키마](upgrades/64-bit-tables.md)
  + 보안 {#security}
    + {hide-from-toc}[고객 자격 증명 순환](security/credential-rotation-guide.md)
    + {hide-from-toc}[비동기 암호화](security/asymetric-encryption.md)
+ [&lt; Campaign v8 설명서로 이동](https://experienceleague.adobe.com/ko/docs/campaign/campaign-v8/campaign-home)
